...
class SQLBreakdownGenPostProcessor:
    """
    This component processes SQL breakdowns generated by the system, cleans up the results,
    constructs the SQL query, and verifies its executability. It utilizes the engine for SQL execution.
    
    Attributes:
    - engine (Engine): The engine responsible for executing SQL queries.
    """
    ...
    async def run(
        ...
        """
        Processes the SQL breakdown replies, cleans the generation result, ensures correct SQL format,
        and validates whether the final SQL query is executable.

        Parameters:
        - replies (List[str]): A list of SQL breakdown replies.
        - project_id (str | None): The project identifier to run the SQL in the context of, if any.

        Returns:
        - Dict[str, Any]: A dictionary containing the cleaned results with SQL steps and description.
        """
        ...
        # Ensure the last step has an empty cte_name
        steps[-1]["cte_name"] = ""
    ...
    def _build_cte_query(self, steps) -> str:
        """
        Builds the final SQL query by combining steps into a Common Table Expression (CTE) query.

        Parameters:
        - steps: A list of steps that define the SQL query.

        Returns:
        - str: The final CTE-based SQL query.
        """
        ctes = ",\n".join(
    ...
    async def _check_if_sql_executable(
        ...
        """
        Checks if the constructed SQL query is executable by sending it to the engine for validation.

        Parameters:
        - sql (str): The SQL query to check.
        - project_id (str | None): The project context for running the SQL, if applicable.

        Returns:
        - bool: True if the SQL is executable, False otherwise.
        """
...
class SQLGenPostProcessor:
    """
    This component processes the SQL generation results, classifies them as valid or invalid,
    and ensures that the generated SQL is executable by the engine. It also handles error scenarios,
    such as adding missing quotes to SQL and performing a dry run check for validity.
    
    Attributes:
    - engine (Engine): The engine responsible for executing and validating SQL queries.
    """
    ...
    async def run(
        ...
        """
        Processes the generated SQL replies and classifies them into valid and invalid results.

        Parameters:
        - replies (List[str]): The SQL generation results from the model.
        - project_id (str | None): The project identifier for context, if applicable.

        Returns:
        - dict: A dictionary containing lists of valid and invalid generation results.
        """
        try:
    ...
    async def _classify_invalid_generation_results(
        ...
        """
        Classifies the generation results as valid or invalid by attempting to execute them and
        checking for errors.

        Parameters:
        - generation_results (List[Dict[str, str]]): The generated SQL results.
        - project_id (str | None): The project context for executing the SQL queries.

        Returns:
        - List[Optional[Dict[str, str]]]: A tuple containing valid and invalid SQL generation results.
        """
        valid_generation_results = []
        invalid_generation_results = []
...
"""
This Next block of code defines rules and a system prompt for a text-to-SQL generation model,
ensuring correct SQL generation by following specific rules, utilizing calculated fields and metrics from the schema, and simplifying complex queries.
It includes schema interpretation, calculated fields, and metric structures to guide SQL query formation based on user inputs and schema structure.
"""
TEXT_TO_SQL_RULES = """
### ALERT ###
